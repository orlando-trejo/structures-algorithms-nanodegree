...
def get_distance(coords, pos_i, pos_f):
    ...
    Inputs: dictionary of coordinates, labels for
    intial and final positions

    Output: distance between two points (float)
    '''
...
# Function to find shortest path between start and goal
...
def shortest_path(roads, intersections, start, goal):
    ...
    Inputs: Graph containing nodes, location of nodes, and
    how nodes are connected

    Output: Path list of optimal path
    '''
    ...
    #Initiate visited and distance dictionaries
    ...
    while not frontier.empty():
        ...
        for node in roads[current]:
            g = current_cost[current] + get_distance(intersections, current, node)

            if node not in current_cost or g < current_cost[node]:
                current_cost[node] = g
                f = g + get_distance(intersections, node, goal)
                frontier.put(node, f)
...
print(shortest_path(roads, intersections, 8, 24))
